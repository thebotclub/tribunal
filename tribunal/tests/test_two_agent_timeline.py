...
def _run_claude_session(client: TestClient, repo: str = "/repo") -> int:
    ...
    post(
        [
            {"hook_event_name": "SessionStart", "session_id": "cc-1", "cwd": repo,
             "claude_code_version": "0.2.10", "source": "user"},
            {"hook_event_name": "UserPromptSubmit", "session_id": "cc-1", "cwd": repo,
             "prompt": "implement add()"},
            {"hook_event_name": "PreToolUse", "session_id": "cc-1", "cwd": repo,
             "tool_name": "Write", "tool_input": {"file_path": "/repo/add.py", "content": "def add(a,b): return a+b"}},
            {"hook_event_name": "PostToolUse", "session_id": "cc-1", "cwd": repo,
             "tool_name": "Write", "tool_input": {"file_path": "/repo/add.py", "content": "def add(a,b): return a+b"},
             "tool_response": {}, "success": True},
            {"hook_event_name": "Stop", "session_id": "cc-1", "cwd": repo,
             "stop_reason": "user", "turn_count": 2},
        ]
    )
...
def _run_cursor_session(client: TestClient, repo: str = "/repo") -> int:
    ...
    cursor.translate(
        {"type": "chat.open", "session_id": "cur-1", "workspace_root": repo,
         "cursor_version": "0.40.1", "model": "gpt-4o"},
        emit,
    )
    cursor.translate(
        {"type": "user.message", "session_id": "cur-1", "workspace_root": repo,
         "text": "fix the failing test"},
        emit,
    )
    cursor.translate(
        {"type": "tool.call", "session_id": "cur-1", "workspace_root": repo,
         "tool": "shell", "args": {"command": "pytest -q"}},
        emit,
    )
    cursor.translate(
        {"type": "tool.result", "session_id": "cur-1", "workspace_root": repo,
         "tool": "shell", "ok": True,
    ...
    cursor.translate(
        {"type": "cost.recorded", "session_id": "cur-1", "workspace_root": repo,
         "cost": {"usd": 0.04, "model": "gpt-4o", "input_tokens": 200, "output_tokens": 80}},
        emit,
    )
    r = client.post("/v1/events", json={"events": sent})
    assert r.status_code == 200
...
def test_two_agents_share_one_timeline(env: tuple[EventStore, TestClient]) -> None:
    ...
    assert any(
        e["agent"] == "claude-code" and e["event_type"] == "file.write"
        for e in events
    )
...
def test_cost_endpoint_aggregates_across_agents(env: tuple[EventStore, TestClient]) -> None:
    _, client = env
    _run_claude_session(client)
    _run_cursor_session(client)
